...
load('extracted_feature1.mat');

%% Fit into knn classifier
% membuat partisi kfold cross validation
fitur1 = extracted_feature1(:,1:124);
labelTrain1 = extracted_feature1(:,125);
cv = cvpartition(labelTrain1, 'KFold', 10);

% train semua data untuk setiap partisi
for i = 1:cv.NumTestSets
    trainPartition = fitur1(cv.training(i),:);
    labelTrainPartition = labelTrain1(cv.training(i),:);
    testPartition = fitur1(cv.test(i),:);
    labelTestPartition = labelTrain1(cv.test(i),:);
...
% save model
saveCompactModel(svmMdl_Linear, 'SVMModel_1_Linear');
saveCompactModel(svmMdl_RBF, 'SVMModel_1_RBF');
saveCompactModel(svmMdl_Polynomial, 'SVMModel_1_Polynomial');
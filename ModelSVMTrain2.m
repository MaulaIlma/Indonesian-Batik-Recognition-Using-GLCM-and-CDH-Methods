...
load('extracted_feature2.mat');

%% Fit into classifier
% membuat partisi kfold cross validation
fitur2 = extracted_feature2(:,1:124);
labelTrain2 = extracted_feature2(:,125);
cv = cvpartition(labelTrain2, 'KFold', 10);

% train semua data untuk setiap partisi
for i = 1:cv.NumTestSets
    trainPartition = fitur2(cv.training(i),:);
    labelTrainPartition = labelTrain2(cv.training(i),:);
    testPartition = fitur2(cv.test(i),:);
    labelTestPartition = labelTrain2(cv.test(i),:);
...
% save model
saveCompactModel(svmMdl_Linear, 'SVMModel_2_Linear');
saveCompactModel(svmMdl_RBF, 'SVMModel_2_RBF');
saveCompactModel(svmMdl_Polynomial, 'SVMModel_2_Polynomial');